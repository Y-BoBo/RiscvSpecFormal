...
then
    ...
    #cmd="./Haskell/Main boot_rom=boot_ROM_RV${xlen}.hex $interactive $noprint testfile=$hexfile tohost_address:$tohost_address xlen@${xlen} $signature $sign_size $debug $interrupts +RTS -p > $dump/$base.out"
    #echo $tohost_address
    #cmd="./Haskell/Main boot_rom=boot_ROM_RV${xlen}.hex $interactive --debug $noprint testfile=$hexfile tohost_address:$tohost_address xlen@${xlen} $signature $sign_size $debug $interrupts > $dump/$base.out"
    cmd="./Haskell/CoqMain boot_rom=boot_ROM_RV${xlen}.hex $interactive --debug $noprint testfile=$hexfile tohost_address:$tohost_address xlen@${xlen} $signature $sign_size $debug $interrupts > $dump/$base.out"
fi
